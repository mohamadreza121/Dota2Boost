  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="surface rounded-[1.7rem] p-5 sm:p-8" noValidate>
      <div className="grid gap-x-5 gap-y-5 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold sm:col-span-2">Ranked party and service experience<textarea className={`${fieldClass} min-h-28 py-3`} placeholder="Tell us about high-rank party play, customer-facing work, and how you communicate under pressure." {...register("coachingExperience")} /><FieldError message={errors.coachingExperience?.message} /></label>
        ...
        <label className="text-xs font-bold">Preferred compensation<input className={fieldClass} placeholder="Per win, package, or queue block" {...register("preferredCompensation")} /><FieldError message={errors.preferredCompensation?.message} /></label>
        ...
        <label className="text-xs font-bold sm:col-span-2">Sample queue plan<textarea className={`${fieldClass} min-h-40 py-3`} placeholder="Using a public match or draft, explain how you would choose roles, hero pairings, communication, and safe queue milestones." {...register("sampleReplayAnalysis")} /><FieldError message={errors.sampleReplayAnalysis?.message} /></label>
        <label className="text-xs font-bold sm:col-span-2">Sample gameplay video URL <span className="font-normal text-mist">(optional)</span><input className={fieldClass} type="url" placeholder="Private unlisted link" {...register("sampleCoachingVideoUrl")} /><FieldError message={errors.sampleCoachingVideoUrl?.message} /></label>
      ...
      <label className="mt-6 flex items-start gap-3 rounded-xl border border-white/[0.09] bg-black/15 p-4 text-xs leading-5 text-mist"><input className="mt-1 accent-[#d25353]" type="checkbox" {...register("agreement")} />I confirm that the information is accurate, I will play only from my own accounts, never request customer credentials or remote access, avoid prohibited tools and manipulation, and follow the booster conduct standards.</label><FieldError message={errors.agreement?.message} />
