...
export default async function SignInPage({ searchParams }: Props) {
  ...
  const audience = query.type === "coach" ? "Coach" : query.type === "admin" ? "Admin" : "Customer";
  return <AuthCard eyebrow={`${audience} access`} title="Welcome back." description="Continue to your private coaching workspace.">
    <div className="mb-6 grid grid-cols-3 gap-1 rounded-xl border border-white/[0.08] bg-black/15 p-1">{[["Customer", ""], ["Coach", "coach"], ["Admin", "admin"]].map(([label, type]) => <Link key={label} href={`/auth/sign-in${type ? `?type=${type}` : ""}`} className={`rounded-lg px-2 py-2 text-center text-[0.65rem] font-bold ${audience === label ? "bg-white/[0.09] text-white" : "text-mist"}`}>{label}</Link>)}</div>
