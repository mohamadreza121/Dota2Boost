...
export default async function SignUpPage({ searchParams }: Props) { const query = await searchParams; return <AuthCard eyebrow="Customer registration" title="Build your workspace." description="Public registration creates customer accounts only. Coaches apply separately and staff accounts are invitation-only." footer={<>Already have an account? <Link href="/auth/sign-in" className="font-bold text-white">Sign in</Link></>}>
  ...
  <p className="mt-5 text-center text-[0.65rem] leading-5 text-mist">Are you a coach? <Link href="/work-with-us" className="font-bold text-cyan">Apply through Work With Us</Link>.</p>
  </AuthCard>; }
