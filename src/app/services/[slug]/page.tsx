...
export default async function ServicePage({ params }: Props) {
  ...
  return (
    <><section className="relative overflow-hidden border-b border-white/[0.07] py-20 md:py-28"><div aria-hidden="true" className="hero-poster absolute inset-0 opacity-20" /><div aria-hidden="true" className="absolute inset-0 bg-[linear-gradient(90deg,#070909_20%,rgb(7_9_9_/_0.72),#070909)]" /><div className="container-shell relative grid gap-12 lg:grid-cols-[1fr_380px] lg:items-end"><div><Badge tone={service.accent === "amber" ? "gold" : service.accent === "cyan" ? "cyan" : "red"}>{service.eyebrow}</Badge><h1 className="display-type mt-6 text-balance text-[clamp(4.1rem,10vw,8rem)] font-black uppercase">{service.name}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-mist">{service.description}</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><LinkButton href="/pricing" arrow>Configure this boost</LinkButton><LinkButton href="/boosters" variant="secondary">Browse boosters</LinkButton></div>{service.slug === "mmr-boost" ? <div className="mt-9 overflow-x-auto pb-2"><div className="rank-medal-track grid min-w-[680px] grid-cols-8 gap-2">{rankFamilies.map((rank) => <RankMedal key={rank} rank={rank} size="sm" />)}</div></div> : null}</div><div className="surface rounded-2xl p-6"><p className="text-[0.62rem] font-bold tracking-wider text-mist uppercase">Packages from</p><p className="mt-2 text-4xl font-black">{formatCurrency(service.priceFrom)}<span className="ml-2 text-sm font-normal text-mist">CAD</span></p><p className="mt-4 text-sm text-mist">Typical scope · {service.duration}</p><div className="mt-5 border-t border-white/[0.08] pt-5 text-xs leading-5 text-[#9fa7a4]">Final price, tax, rank compatibility, and booster availability are verified securely before delivery.</div></div></div></section>
