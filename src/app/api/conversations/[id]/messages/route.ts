...
export async function POST(request: Request, { params }: Context) {
  ...
  const { data, error } = await supabase.from("messages").insert({ conversation_id: id, sender_id: user.id, client_id: parsed.data.clientId, body: parsed.data.body, kind: parsed.data.kind, reply_to_id: parsed.data.replyTo ?? null }).select("id, client_id, body, kind, created_at").single();
